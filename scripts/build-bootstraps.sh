...
TERMUX_BUILT_DEBS_MULTIARCH_DIRECTORY="$TERMUX_PACKAGES_DIRECTORY/output"
TERMUX_BUILT_DEBS_DIRECTORY="$TERMUX_BUILT_DEBS_MULTIARCH_DIRECTORY/current-arch"
...
build_bootstrap_trap() {
	...
	[ -h "$TERMUX_BUILT_DEBS_DIRECTORY" ] && rm -f "$TERMUX_BUILT_DEBS_DIRECTORY"
...
main() {
	...
	for package_arch in "${TERMUX_ARCHITECTURES[@]}"; do
		...
		TERMUX_BUILT_DEBS_DIRECTORY_FOR_ARCH="$TERMUX_BUILT_DEBS_MULTIARCH_DIRECTORY/arch-$package_arch"
		mkdir -p "$TERMUX_BUILT_DEBS_DIRECTORY_FOR_ARCH"

		if [ -f "$TERMUX_BUILT_PACKAGES_DIRECTORY" ] || [ -d "$TERMUX_BUILT_PACKAGES_DIRECTORY" ]; then
			rm -rf "$TERMUX_BUILT_PACKAGES_DIRECTORY"
		fi
		if [ -f "$TERMUX_BUILT_DEBS_DIRECTORY" ] || [ -d "$TERMUX_BUILT_DEBS_DIRECTORY" ]; then
			rm -rf "$TERMUX_BUILT_DEBS_DIRECTORY"
		fi

		ln -sf "$TERMUX_BUILT_PACKAGES_DIRECTORY_FOR_ARCH" "$TERMUX_BUILT_PACKAGES_DIRECTORY"
		ln -sf "arch-$package_arch" "$TERMUX_BUILT_DEBS_DIRECTORY"
